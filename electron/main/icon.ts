...
export function ensureLinuxDevDesktopEntry(): void {
  ...
  const body = [
    ...
    `Exec=${JSON.stringify(process.execPath)} ${JSON.stringify(process.argv[1] || '')} %u`,
    `Icon=${icon}`,
    'Terminal=false',
    'Categories=Finance;Utility;',
    'MimeType=x-scheme-handler/wc;x-scheme-handler/walletconnect;x-scheme-handler/bee-wallet;',
